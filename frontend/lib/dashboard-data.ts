...
export type DiscoveryPipelineOverview = {
  ...
  domain_count: number;
  entity_domain_count: number;
  top_domains: Array<{ domain: string; count: number }>;
  candidate_domain_count: number;
  top_candidate_domains: Array<{ domain: string; count: number }>;
...
async function getDiscoveryPipelineCandidates(): Promise<PipelineCandidate[]> {
  try {
    const response = await fetch(`${apiBaseUrl}/api/v1/discovery-pipeline/candidates?limit=100`, {
      cache: "no-store"
    });
    if (!response.ok) {
      return [];
